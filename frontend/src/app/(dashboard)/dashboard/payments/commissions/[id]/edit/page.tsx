...
export default function EditCommissionPage({ params }: PageProps) {
  const resolvedParams = use(params);
  const router = useRouter();
  const commissionId = parseInt(resolvedParams.id);
  const { useCommission, updateCommission, isUpdating } = useCommissions();
  ...
  const form = useForm({
    ...
    values: commission
      ? {
          ...
          partner_operator_id: commission.partner_operator_id,
  ...
  const onSubmit = async (data: CommissionFormData) => {
    try {
      ...
      await updateCommission({ id: commissionId, data: processedData });
      router.push(`/dashboard/payments/commissions/${commissionId}`);
    } catch (error) {
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold">Edit Commission</h1>
          <p className="text-muted-foreground">Update commission information</p>
        </div>
      </div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit as any)} className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Associated booking details</CardDescription>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Type and recipient details</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <FormField
                control={form.control}
