...
export default function CreateExtraExpensePage() {
  const router = useRouter();
  const { createExtra, isCreating } = useExtras();

  const form = useForm({
    resolver: zodResolver(extraExpenseSchema) as any,
    defaultValues: defaultExtraExpenseValues,
  });
  ...
  const onSubmit = async (data: ExtraExpenseFormData) => {
    try {
      ...
      console.error('Failed to create extra expense:', error);
    }
  };
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold">Add New Extra Expense</h1>
          <p className="text-muted-foreground">Create a new extra expense in your inventory</p>
        </div>
      </div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit as any)} className="space-y-6">
          ...
          <div className="flex justify-end gap-4">
            ...
            <Button type="submit" disabled={isCreating}>
              ...
              {isCreating ? 'Creating...' : 'Create Extra Expense'}
            </Button>
          </div>
        </form>
      </Form>
