...
import {
  b2bClientSchema,
  defaultB2BClientValues,
  B2BClientFormData,
} from '@/lib/validations/b2b-clients';
...
export default function CreateB2BClientPage() {
  const router = useRouter();
  const { createB2BClient, isCreating } = useB2BClients();
  const { operators } = useOperators();

  const form = useForm({
    resolver: zodResolver(b2bClientSchema) as any,
    defaultValues: defaultB2BClientValues,
  });
  ...
  const onSubmit = async (data: B2BClientFormData) => {
    try {
      const processedData = {
        ...
        partnerOperatorId: data.partner_operator_id || undefined,
      ...
      await createB2BClient(processedData);
      router.push('/dashboard/clients/b2b');
    } catch (error) {
      console.error('Failed to create B2B client:', error);
    }
  };
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold">Add New B2B Client</h1>
          <p className="text-muted-foreground">Create a new business client</p>
        </div>
      </div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit as any)} className="space-y-6">
          ...
          <div className="flex justify-end gap-4">
            ...
            <Button type="submit" disabled={isCreating}>
              ...
              {isCreating ? 'Creating...' : 'Create B2B Client'}
            </Button>
          </div>
        </form>
      </Form>
