...
export default function B2CClientDetailsPage() {
  const router = useRouter();
  const params = useParams();
  const clientId = parseInt(params.id as string);
  const { useB2CClient, deleteB2CClient, isDeleting } = useB2CClients();
  const { data: client, isLoading } = useB2CClient(clientId);
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);

  const handleDelete = async () => {
    await deleteB2CClient(clientId);
    ...
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button
            variant="outline"
            onClick={() => router.push(`/dashboard/clients/b2c/${clientId}/edit`)}
          >
